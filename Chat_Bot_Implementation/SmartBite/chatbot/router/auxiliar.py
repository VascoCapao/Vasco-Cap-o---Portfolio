...
def sanitize_input(user_input):
    ...
    Returns:
        str: The sanitized input if safe, or an error message if malicious.
    """
    # Define a regex pattern to detect malicious commands or prompt injections
    malicious_patterns = [
        r"(?i)(system\.prompt|delete|shutdown|alter|drop table|insert into)",  # SQL/command injections
        r"<.*?>",  # HTML/Script injections
        r"[\{\}\[\]]",  # Avoid special tokens that could confuse the model
    ]

    for pattern in malicious_patterns:
        if re.search(pattern, user_input):
            return "[Input rejected due to potentially harmful content.]"
    
    # If no malicious patterns are found, return the cleaned input
